    ...
    fun registerListener(listener: Any) {
        for (method in listener::class.java.declaredMethods) {
            ...
            if (method.isAnnotationPresent(Listener::class.java)) {
                ...
                registerListener(listener, anno.value, event, method)
            }
        }
    }

    inline fun <reified E : Event> registerListener(priority: EventPriority, crossinline func: (E) -> Unit): LambdaListener<E> {
        ...
        registerListener(ll, priority, E::class.java, method)
        return ll
    }

    fun registerListener(listener: Any, priority: EventPriority, event: Class<out Event>, method: Method) {
        method.isAccessible = true
        val handler = handlers.getOrPut(event) { EventHandler(event) }

        val wl = WrapListener(event, MethodHandles.lookup().unreflect(method), listener)
