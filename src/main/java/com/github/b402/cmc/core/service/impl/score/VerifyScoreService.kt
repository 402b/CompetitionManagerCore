...
import com.github.b402.cmc.core.event.score.ScoreVerifyEvent
...
object VerifyScoreService : DataService<SubmitData>(
    ...
    override suspend fun onRequest(data: SubmitData): ReturnData {
        ...
        val checked = data.json.getNumberList("checked")
                ?: return returnData(ILLEGAL_INPUT, "传入参数不足")
        ...
        for (num in checked) {
            ...
            jobs += GlobalScope.launch(coroutineContext) {
                ...
                if (verify) {
                    ...
                    val sve = ScoreVerifyEvent(score)
                    EventBus.callEvent(sve)
                    if (!sve.isCancel)
                        score.sync().await()
                } else {
                    score.remove().await()
